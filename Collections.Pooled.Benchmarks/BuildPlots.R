# Copy this file to the BenchmarkDotNet.Artifacts\results directory and run it with rscript.

# This is based on the RPlotsExporter but removes most of the charts to save time
# and adds charting of memory usage.
...
for (file in files) {
  ...
  sizeUnit <- "Bytes"
  if (min(result$Allocated_Bytes) > 1000) {
    result$Allocated_Bytes <- result$Allocated_Bytes / 1000
    sizeUnit <- "Kilobytes"
  }
  if (min(result$Allocated_Bytes) > 1000) {
    result$Allocated_Bytes <- result$Allocated_Bytes / 1000
  ...
  resultStats <- result %>%
    group_by(.dots = c("Target_Method", "Job_Id")) %>%
    summarise(se = std.error(Measurement_Value), Value = mean(Measurement_Value), Allocated = mean(Allocated_Bytes))
  ...
  benchmarkBarplot <- ggplot(resultStats, aes(x=Target_Method, y=Value, fill=Job_Id)) +
    ...
    xlab("Target") +
    ylab(paste("Time:", timeUnit)) +
    ggtitle(paste(title, "- Time")) +
    geom_bar(position=position_dodge(), stat="identity")
    #geom_errorbar(aes(ymin=Value-1.96*se, ymax=Value+1.96*se), width=.2, position=position_dodge(.9))

  memoryBarPlot <- ggplot(resultStats, aes(x=Target_Method, y=Allocated, fill=Job_Id)) +
    guides(fill=guide_legend(title="Job")) +
    xlab("Target") +
    ylab(paste("Allocated ", sizeUnit)) +
    ggtitle(paste(title, "- Memory")) +
    geom_bar(position=position_dodge(), stat="identity")

  #printNice(benchmarkBoxplot)
  printNice(benchmarkBarplot)
  printNice(memoryBarPlot)
  #ggsaveNice(gsub("-measurements.csv", "-boxplot.png", file), benchmarkBoxplot)
  ggsaveNice(gsub("-measurements.csv", "-timeplot.png", file), benchmarkBarplot)
  ggsaveNice(gsub("-measurements.csv", "-memoryplot.png", file), memoryBarPlot)
